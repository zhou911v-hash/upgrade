...
const DECK_ID = "smart-building-lighting-defense";
const OUT_DIR = "/Users/wxf/Desktop/周文旭毕设/SmartBuildingLighting/docs/presentation-output";
const REF_DIR = "/Users/wxf/Desktop/周文旭毕设/SmartBuildingLighting/tmp/slides/smart-building-lighting-defense/ref";
...
const SOURCES = {
  primary: "来源: 课题任务书、开题报告、SmartBuildingLighting 项目代码与当前实现结果。",
};

const SLIDES = [
  {
    kicker: "GRADUATION DESIGN",
    title: "基于 WPF 的智能楼宇照明监控系统的设计与实现",
    subtitle: "面向楼宇照明的实时监测、可视化控制、定时调度与能耗分析平台。",
    expectedVisual: "Cover slide with strong title and one core outcome callout.",
    moment: "WPF + MVVM + SQLite + Modbus TCP",
    notes: "封面页说明课题名称、定位与整体技术路线。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 02",
    title: "课题背景与研究意义",
    subtitle: "智能楼宇建设与节能减排需求推动照明系统从人工控制走向可视化与智能化。",
    expectedVisual: "Three supporting cards describing background, pain points, and project value.",
    cards: [
      ["行业背景", "商业建筑中照明能耗占比较高，传统方式存在管理分散、能耗浪费和状态不透明的问题。"],
      ["现实痛点", "人工巡检效率低、联动控制能力弱、缺少历史数据支撑，无法满足现代楼宇精细化管理需求。"],
      ["课题价值", "通过软件平台实现监测、控制、调度和统计一体化，提升管理效率并支撑节能目标。"]
    ],
    notes: "说明为什么要做这个系统，以及该系统在智慧楼宇场景中的实际意义。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 03",
    title: "系统需求与设计目标",
    subtitle: "在功能、可视化和可扩展性上同时满足毕业设计与工程演示要求。",
    expectedVisual: "Metric-style design goal cards.",
    metrics: [
      ["5 层", "楼层示意范围", "覆盖楼层、区域与照明回路层级"],
      ["97 个", "预置照明回路", "支持平面图与三维视图联动控制"],
      ["4 类", "核心控制对象", "单回路、分组、情景模式、全楼"]
    ],
    notes: "重点强调实时监控、分组控制、情景模式、定时调度和扩展能力。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 04",
    title: "系统总体架构设计",
    subtitle: "采用分层架构隔离界面、业务、数据与通信，实现清晰职责划分。",
    expectedVisual: "Architecture cards explaining each layer.",
    cards: [
      ["表现层", "WPF + MVVM 实现桌面交互，负责仪表盘、2D/3D 监控、分组、情景、调度、统计等页面。"],
      ["业务层", "LightCircuitService、GroupService、SceneModeService、ScheduleService、EnergyService 共同实现控制闭环。"],
      ["基础设施层", "SQLite + EF Core 负责持久化，CommunicationService 统一通信入口，Quartz 承担后台调度。"]
    ],
    notes: "可结合 FigJam 架构图进一步说明层级关系与运行时协作方式。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 05",
    title: "数据库与核心实体设计",
    subtitle: "围绕楼层、区域、回路、控制策略与运行记录组织核心数据模型。",
    expectedVisual: "Entity design cards.",
    cards: [
      ["空间实体", "Floor、Area、LightCircuit 描述楼宇结构与照明回路位置，为 2D/3D 视图提供布局基础。"],
      ["控制实体", "LightGroup、SceneMode、Schedule 描述批量控制、情景模式与定时调度策略。"],
      ["运行实体", "EnergyRecord、OperationLog、CommunicationProfile、ScheduleExecutionRecord 记录能耗、日志、通信配置与调度执行结果。"]
    ],
    notes: "突出新增通信配置与调度执行记录两个实体，说明其对演示闭环的重要性。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 06",
    title: "2D / 3D 可视化监控实现",
    subtitle: "系统同时支持楼层平面图监控与三维楼层状态展示，两种视图共用同一状态源。",
    expectedVisual: "Metrics describing two visualization modes.",
    metrics: [
      ["2D", "平面图监控", "基于区域坐标与回路相对坐标绘制楼层示意图"],
      ["3D", "三维监控", "基于 HelixToolkit 组合楼层体块、区域盒体与灯位节点"],
      ["1 套", "统一状态源", "通信事件回写后，2D 与 3D 界面同步刷新"]
    ],
    notes: "说明 2D 更适合定位区域，3D 更适合整体展示楼层态势，两者形成互补。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 07",
    title: "分组、情景与定时调度",
    subtitle: "系统将常见楼宇照明控制需求抽象为三类核心策略能力。",
    expectedVisual: "Three control strategy cards.",
    cards: [
      ["分组控制", "管理员可按楼层、功能区或业务场景组合回路，并对整组执行开关和亮度控制。"],
      ["情景模式", "支持预设模式与自定义模式，能够为不同时间段或使用情境快速切换照明状态。"],
      ["定时调度", "Quartz 按 Cron 执行单回路、分组、情景模式或全楼任务，并同步写入执行记录。"]
    ],
    notes: "这是系统控制闭环的核心页，适合答辩时配合界面录屏讲解。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 08",
    title: "能耗统计与日志追踪",
    subtitle: "平台不只做控制，也记录运行痕迹与能耗变化，为节能分析提供数据基础。",
    expectedVisual: "Three cards explaining energy and logging.",
    cards: [
      ["能耗采样", "系统根据通信回写与周期采样累计能耗记录，形成日趋势、楼层对比与回路排行。"],
      ["图表展示", "仪表盘与统计页使用原生图表展示趋势和对比信息，支持时间范围筛选与 CSV 导出。"],
      ["日志追踪", "所有关键操作和调度执行都会写入操作日志，支持分页查询与时间过滤。"]
    ],
    notes: "强调系统具备从控制到追踪再到分析的完整数据链条。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 09",
    title: "技术难点与解决方案",
    subtitle: "围绕跨平台构建、双模式通信、后台调度和三维展示解决关键实现问题。",
    expectedVisual: "Three metric-style solution cards.",
    metrics: [
      ["双模式", "通信门面", "统一封装模拟器与 Modbus TCP，减少上层业务差异"],
      ["后台执行", "调度隔离", "共享调度器 + 独立作用域执行，避免 DbContext 跨线程风险"],
      ["跨平台", "构建验证", "非 Windows 环境完成构建与测试，Windows 负责最终 UI 运行验收"]
    ],
    notes: "如果现场被问到工程性问题，这一页是主要回答支点。",
    sources: ["primary"]
  },
  {
    kicker: "SECTION 10",
    title: "总结与展望",
    subtitle: "系统已完成毕业设计目标中的主要功能，并具备继续工程化拓展的基础。",
    expectedVisual: "Closing summary cards.",
    cards: [
      ["已完成成果", "完成 WPF 桌面系统、测试工程、Figma 设计稿、FigJam 架构图与本地答辩演示文稿。"],
      ["系统特点", "具备可视化、可控制、可调度、可统计、可扩展等特点，满足毕业设计演示要求。"],
      ["后续展望", "可继续扩展真实控制器映射、故障告警、移动端协同、更精细能耗分析与历史回放。"]
    ],
    notes: "最后归纳系统成果，并给出具有延展性的后续方向。",
    sources: ["primary"]
  }
];
